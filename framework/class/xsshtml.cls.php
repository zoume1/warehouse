...
//todo 支持优酷视频等
class XssHtml {
	public static function clean_xss($html, $AllowTag = array()) {
		$x = new XssHtml($html, 'utf-8', $AllowTag);
		$ret = $x->getHtml();
	...
	private $m_AllowTag = array('a', 'img', 'br', 'strong', 'small', 'b', 'code', 'pre', 'p', 'div', 'em', 'span', 
				'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'table', 'ul', 'ol', 'tr', 'th', 'td', 'hr', 'li', 'u', 
				'section', 'article', 'nav', 
				'embed', 'video', 'audio');
	...
	private function __true_url($url){
		if (preg_match('#^https?://.+#is', $url)) {
			return $url;
		}else if (preg_match('#^tel:.+#is', $url)) {
			return $url;
		}else{
			return  (isHttps() ? 'https://' : 'http://').getDomainName(). $url;
		}
	}
	...
	private function __node_embed($node){
		...
		if((!$url = parse_url($link)) || empty($url['host']) ||
			!in_array($url['host'], $white_list)) {
			//print_r($node->c14n());die;
			$replace = new DomElement('p', '抱歉！只支持优酷和腾讯视频！warning! embed is not allowed from '.htmlspecialchars($link));
			@$node->parentNode->replaceChild($replace, $node);
			//$this->m_dom->removeChild($node);
			return false;
		}
	...
	private function __node_video($node){
		$this->__common_attr($node);

		$this->__set_default_attr($node, 'src');
		$this->__set_default_attr($node, 'autoplay');
		$this->__set_default_attr($node, 'loop');
		$this->__set_default_attr($node, 'controls');
	}

	private function __node_audio($node){
		$this->__common_attr($node);
