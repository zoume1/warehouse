...
/* 前后端通信相关的配置,注释只允许使用多行方式 */
    ...
    'scrawlMaxSize'=> 20971520, /* 上传大小限制，单位B */
	...
	public function index() {
		...
		switch($action) {
			case 'config':
				$result = json_encode($this->config_json);
				break;
		    ...
		    /* 上传文件 */
		    case 'uploadfile':
				if($result = UploadMod::do_upload_one($_FILES['file'])) {
					$result['state'] = 'SUCCESS';
					$result['original'] = $result['file_name'];
					$result['title'] = $result['file_name'];
					$result['type'] = strrchr($result['file_name'], '.');
					$result['size'] = $result['file_size'];
